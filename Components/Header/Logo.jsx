import React from "react";

const Logo = ({ className }) => {
  return (
    <svg className={className} version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 235.6 68">
      <g id="Markenzeichen">
        <g>
          <polygon fill="currentColor" points="46,18.8 10.5,18.8 10.5,0 0,0 0,28.3 56.6,28.3 56.6,0 46,0 " />
          <path
            fill="currentColor"
            d="M235.6,14.2C235.6,5.6,230,0,220.1,0h-42.9v28.4h42.9C229.9,28.3,235.6,22.8,235.6,14.2z M187.7,18.8V9.6
            h31.9c3.2,0,5.5,1.5,5.5,4.6s-2.2,4.6-5.5,4.6L187.7,18.8L187.7,18.8z"
          />
          <polygon fill="currentColor" points="0,68 10.5,68 10.5,49.3 46,49.3 46,68 56.6,68 56.6,39.7 0,39.7 " />
          <polygon fill="currentColor" points="90.6,39.7 90.6,68 101.2,68 101.2,50 136.1,68 157.3,68 101.2,39.7 " />
          <path
            fill="currentColor"
            d="M220.1,39.7h-42.9V68h42.9c9.9,0,15.5-5.6,15.5-14.2S229.9,39.7,220.1,39.7z M219.6,58.5h-31.9v-9.2h31.9
            c3.2,0,5.5,1.5,5.5,4.6C225.1,57,222.8,58.5,219.6,58.5z"
          />
          <polygon fill="currentColor" points="101.2,18 101.2,0 90.6,0 90.6,28.3 101.2,28.3 157.3,0 136.1,0 " />
        </g>
      </g>
    </svg>
  );
};

export default Logo;
