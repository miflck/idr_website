...
export const EDITORIALEINZELN = ` 
query EditorialBySlug($slug: String!,$locale: SiteLocale) {
  editorial(locale: $locale, filter: {slug: {eq: $slug}}) {
    id
    reihenfolge
    title
    slug
...
{
  allMenschens (first:100,filter: {extern: {eq: "false"},aktiv: {eq: "true"}}){
    id
    name
    slug
  }
...
export const MENSCHEN = `
  query Menschen($locale: SiteLocale) {
    allMenschens(first:100, locale: $locale, orderBy: nachname_ASC, filter: {extern: {eq: "false"}}) {
      name
      aktiv
      email
      extern
...
export const ALLPROJECTSBYFORSCHUNGSFELD = `
query AllProjektsByForschungsfeld( $locale: SiteLocale,$forschungsfeldId: [ItemId]!) {
  
  allProjekts(first:100,locale:$locale,filter: {forschungsfeld: {eq: $forschungsfeldId}}) {
    id
  titel
      forschungsfeld {
      titel
