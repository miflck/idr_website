...
{
  allMenschens (first:999,filter: {extern: {eq: "false"},aktiv: {eq: "true"}}){
    id
    name
    slug
  }
...
export const MENSCHEN = `
  query Menschen($locale: SiteLocale) {
    allMenschens(first:999, locale: $locale, orderBy: nachname_ASC, filter: {extern: {eq: "false"}}) {
      name
      aktiv
      email
      extern
...
export const MENSCHEINZEL = `
  query MenschBySlug($slug: String!, $locale: SiteLocale) {
    allProjekts (first:999, locale: $locale,orderBy: enddatum_DESC) {
      titel
      slug
      mitarbeit {
        name
